...
contract Verifier{
    ...
    function setRole(address _addr, roles _role, uint _amt) public returns(bool) {
...
contract PdtDetails is Verifier{
    ...
    mapping(address => mapping(bytes32 => Product)) PQuantity_tracker;
    mapping(address => mapping(bytes32 => Product)) DQuantity_tracker;
    mapping(address => mapping(bytes32 => Product)) RQuantity_tracker;
   ...
   function producerEnterProductDetails(bytes32 id,bytes32 _producer_product_name,bytes32 _info_organic, 
    bytes32 _producer_product_info, uint _producer_product_quantity, uint _producer_product_price, conditions _p_in) 
   public returns(bool)
   {
        
        ProductDB[id].producer_product_name = _producer_product_name;
        ProductDB[id].info_organic = _info_organic;
        PQuantity_tracker[msg.sender][id].producer_product_quantity = _producer_product_quantity;
        ...
        constant public returns(bytes32,bytes32,bytes32,uint,uint,uint)
    {
        ...
        return(ProductDB[id].producer_product_name,
            ...
            PQuantity_tracker[ProductDB[id].producer_address][id].producer_product_quantity,
    ...
    public returns(bool)
    {
        ...
        require(_quantity<=PQuantity_tracker[_addr][id].producer_product_quantity);
        
        _amount = _quantity * ProductDB[id].producer_product_price;
        PQuantity_tracker[_addr][id].producer_product_quantity -= _quantity;
        DQuantity_tracker[msg.sender][id].distributor_product_quantity +=_quantity;
    ...
    uint _distributor_product_price, conditions _d_in) public returns (bool) 
    {
       
       ProductDB[id].distributor_product_info = _distributor_product_info;
       ProductDB[id].distributor_product_price = _distributor_product_price;
       ProductDB[id].distributor_timestamp = now;
       ProductDB[id].d = _d_in;
       emit LogProductDetailsByDistributor(msg.sender,id,_distributor_product_price,_d_in,
        ...
        constant public returns(bytes32,uint,uint)
    {
        ...
        return(ProductDB[id].distributor_product_info,
               DQuantity_tracker[ProductDB[id].distributor_address][id].distributor_product_quantity, 
   ...
   /// To pay digital token by the retailer if conditions are meet 
    ...
    function paymentByRetailer(bytes32 id, uint _quantity, address _addr, conditions _d_in) 
    public returns (bool)
    {
            uint _amount;
            ...
            require(_quantity<=DQuantity_tracker[_addr][id].distributor_product_quantity);
        
            _amount = _quantity * ProductDB[id].distributor_product_price; 
            DQuantity_tracker[_addr][id].distributor_product_quantity -= _quantity;
            RQuantity_tracker[msg.sender][id].retailer_product_quantity +=_quantity;
    ...
    function enterRetailerDetails(bytes32 id, bytes32 _retailer_product_info, 
                                uint _retailer_product_price, conditions _r_in) 
                                 public returns(bool)
    {
        
        ProductDB[id].retailer_product_price = _retailer_product_price;
    ...
    /// input : product id 
    /// output : product details 
    function getProductInfoByRetailer2(bytes32 id) constant public returns(bytes32,uint,uint)
    {
        assert((verified_address[msg.sender] == roles.Producer) || 
        (verified_address[msg.sender] == roles.Distributor) || 
        (verified_address[msg.sender] == roles.Retailer));
        ...
        return (ProductDB[id].retailer_product_info,
                RQuantity_tracker[ProductDB[id].retailer_address][id].retailer_product_quantity, 
