...
contract PdtDetails is Verifier{
    ...
    event LogProductDetailsByDistributor(address ADDRESS,bytes32 INFO,uint VALUE,uint PRICE,conditions,string);
    ...
    event LogProductDetailsByRetailer(address ADDRESS,bytes32 INFO,string);
   ...
   onlyProducer(msg.sender) public returns(bool)
   {
    ...
    function enterDistributorDetails(bytes32 id, bytes32 _distributor_product_info, uint _distributor_product_value,uint _distributor_product_price, conditions _d_in)
    onlyDistributor(msg.sender) public returns (bool) {
       ...
       emit LogProductDetailsByDistributor(msg.sender,id,_distributor_product_value,_distributor_product_price,_d_in,
    ...
    function getProductInfoByDistributor2(bytes32 id) 
        constant public returns(bytes32,uint,uint,uint)
    {
        assert((verified_address[msg.sender] == roles.Producer) || 
        (verified_address[msg.sender] == roles.Distributor) || 
        (verified_address[msg.sender] == roles.Retailer));
        ...
        return(ProductDB[id].distributor_product_info,
               ...
               ProductDB[id].distributor_product_price,
    ...
    onlyRetailer(msg.sender) public returns (bool)
    {
           ...
           // blockDetailsMappedtoid[id].paymentByRetailer_block = block.number;
            emit LogPaymentByRetailer(msg.sender,ProductDB[id].producer_product_name,initial_amt[_addr],initial_amt[msg.sender],_addr,
           "Payment By Retailer Confirmed and Contract transferred from Distributor to Retailer");
            return true;
    ...
    function enterRetailerDetails(bytes32 id, bytes32 _retailer_product_info, 
                                uint _retailer_product_value, conditions _r_in) 
                                onlyRetailer(msg.sender) public returns(bool)
    {
    ...
    /// input : product id 
    /// output : product details 
    function getProductInfoByRetailer2(bytes32 id) constant public returns(bytes32,uint,uint,uint)
    {
        assert((verified_address[msg.sender] == roles.Producer) || 
        (verified_address[msg.sender] == roles.Distributor) || 
        (verified_address[msg.sender] == roles.Retailer));
        ...
        return (ProductDB[id].retailer_product_info,
                ...
                ProductDB[id].retailer_product_price,
