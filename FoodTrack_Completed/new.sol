...
contract Verifier{
    ...
    function setRole(uint _count,address _addr, roles _role, uint _amt) onlyBy(chair_person)
     public returns(bool)
    {   
...
contract ProductDetails is Verifier{
    ...
    uint _producer_product_value, uint _exp_date, conditions _p_in) onlyProducer(msg.sender)
    public returns(bool)
    {
    ...
    function dispalyForDistirbutor2() constant public returns(uint[], uint[], bytes32[], address[], 
    bytes32[])
    {
    ...
    function displayForRetailer1() public constant returns(uint[], uint[], bytes32[], uint[], bytes32[],
    address[])
    {
