...
contract ProductDetails is Verifier{
    ...
    event LogPaymentByDistributor(address Distributor_address,bytes32 Product_Name,uint PAmount,uint DAmount,address Producer_Address,string Detail);
   ...
   function producerEnterProductDetails(bytes32 id,bytes32 _producer_product_name,bytes32 _info_organic, 
    bytes32 _producer_product_info, uint _producer_product_value, conditions _p_in) 
   onlyProducer(msg.sender) public returns(bool)
   {
    ...
    function getProductInfoByProducer2(bytes32 id) 
        constant public returns(bytes32,bytes32,bytes32,uint,uint)
    {
        assert((verified_address[msg.sender] == roles.Producer) || 
        (verified_address[msg.sender] == roles.Distributor) || 
        (verified_address[msg.sender] == roles.Retailer));
    ...
    onlyDistributor(msg.sender) public returns(bool)
    {
        ...
        emit LogPaymentByDistributor(msg.sender,ProductDB[id].producer_product_name,initial_amt[_addr],initial_amt[msg.sender],_addr,
        "Payment By Distributor Confirmed and Contract transaferred from Producer to Distributor");
        return true;
    }
    ...
    onlyRetailer(msg.sender) public returns (bool)
    {
        ...
        "Payment By Retailer Confirmed and Contract transaferred from Distributor to Retailer");
            return true;
    }
